...
def dbinit(logger,MySql,bool): #pass in mysql = MySQL(app) from init.py
    if bool==0:
    ...
    profiletable="CREATE TABLE Profile (  user_id integer(16) not null,  name varchar(16),  primary key (user_id)  );     CREATE TABLE interact_with (  mem_username varchar(16),  user_id integer(16),  primary key (mem_username, user_id),  foreign key (mem_username) references Members(mem_username) ON DELETE CASCADE,  foreign key (user_id) references Profile(user_id) ON DELETE CASCADE  );"
    ...
    cur.execute(companytable)
    cur.execute(platformtable)
    cur.execute(gametable)
    cur.execute(releasedontable)
    cur.execute(userstable)
    cur.execute(guestable)
    cur.execute(memberstable)
    cur.execute(requestgame)
    cur.execute(commenttable)
    cur.execute(reviewtable)
    cur.execute(reporttable)
    cur.execute(bookmarktable)
    cur.execute(admintable)
    cur.execute(profiletable)
    cur.execute(managetable)
