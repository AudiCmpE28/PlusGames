...
def dbreinit(logger,connection,bool): #pass in mysql = MySQL(app) from init.py
    ...
    gametable=      "CREATE TABLE Game(game_id integer(10) not null,g_company varchar(25) not null,game_n varchar(50) not null,genre varchar(30),rating decimal(4,2),release_Date	date,price decimal(4,2),primary key (game_id,g_company),foreign key (g_company) references Company(CompanyName));"
    ...
    memberstable=   "create table Members (unique_id integer(16)not null,mem_username varchar(25)not null,mem_email VARCHAR(50),mem_password varchar(200) default null, primary key (unique_id, mem_username),foreign key (unique_id) references Users(unique_id)on update cascade on delete cascade,unique (mem_username));"
    requestgame=    "CREATE TABLE request_game (mem_username varchar(25) not null,game_id integer(10),req_text varchar(250),foreign key (mem_username) references Members(mem_username) ON UPDATE CASCADE ON DELETE CASCADE,foreign key (game_id) references Game(game_id) ON UPDATE CASCADE ON DELETE CASCADE);"
    commenttable=   "CREATE TABLE comment_on (mem_username varchar(25)not null,game_id integer(10)not null,c_date date,c_time time,comment_text varchar(250),primary key (mem_username, game_id),foreign key (mem_username) references Members(mem_username) on delete CASCADE,foreign key (game_id) references Game(game_id) on delete CASCADE);"
    reviewtable=    "CREATE TABLE review_on (mem_username varchar(25)not null,game_id integer(10)not null,rv_date date,rv_time timestamp,review_text varchar(250),primary key (mem_username, game_id), foreign key (mem_username) references Members(mem_username) on delete CASCADE,foreign key (game_id) references Game(game_id) on delete CASCADE);"
    reporttable=    "CREATE TABLE report_on (mem_username varchar(25)not null,game_id integer(10)not null,rp_date date,rp_time timestamp,report_text varchar(250),primary key (mem_username, game_id), foreign key (mem_username) references Members(mem_username) on delete CASCADE,foreign key (game_id) references Game(game_id) on delete CASCADE);"
    bookmarktable=  "CREATE TABLE bookmarked (mem_username varchar(25) not null,game_id integer(10) not null,primary key (mem_username, game_id), foreign key (mem_username) references Members(mem_username) on delete CASCADE,foreign key (game_id) references Game(game_id) on delete CASCADE);"
    admintable=     "CREATE TABLE Administrator (unique_id integer(16) not null,admin_username varchar(25) not null,admin_email varchar(50),admin_password varchar(200) default null,primary key (unique_id, admin_username),foreign key (unique_id) references Users(unique_id)on update cascade on delete cascade,unique (admin_username));"
    profiletable=   "CREATE TABLE Profile (user_id integer(16) not null,name varchar(16),primary key (user_id));"
    interactable=   "CREATE TABLE interact_with (mem_username varchar(25),user_id integer(16),primary key (mem_username, user_id),foreign key (mem_username) references Members(mem_username) ON DELETE CASCADE,foreign key (user_id) references Profile(user_id) ON DELETE CASCADE);"
    managetable=    "CREATE TABLE manage (admin_username varchar(25) not null,user_id integer(16) not null,primary key (admin_username, user_id),foreign key (admin_username) references Administrator(admin_username) ON DELETE CASCADE,foreign key (user_id) references Profile(user_id) ON DELETE CASCADE);"
